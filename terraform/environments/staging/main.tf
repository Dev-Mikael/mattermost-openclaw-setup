# Staging environment — cheap, disposable, for testing changes before production.
# Uses smaller instances and force_destroy=true on S3 so teardown is clean.
...
module "secrets" {
  ...
  environment            = local.environment
  secret_prefix          = var.secret_prefix
  secret_recovery_window = 0 # Allow immediate deletion in staging
}
